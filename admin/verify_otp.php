...
if (isset($_POST['verify'])) {
    ...
    if ($otp == $inpOTP) {
        if (isset($_SESSION['admin_name'])) {
            $loc = "admin_pro.php";
            $npass = $_SESSION['new_pass'];
            $addr = $_SESSION['admin_mail'];
            $up = "UPDATE admins SET password = '$npass' WHERE email = '$addr'";
            ...
            $loc = "";
            $npass = $_SESSION['new_pass'];
            $addr = $_SESSION['user_email'];
            $up = "UPDATE users SET password = '$npass' WHERE email = '$addr'";
            if ($res = mysqli_query($conn, $up)) {
                echo "<script>alert('Password updated sucessfully!')
                window.location = 'http://localhost/GYMAPP/us_profile.php'; </script>";
            }else {
                echo "<script>alert('Something went wrong, please try again!')</script>";
            }
        }
